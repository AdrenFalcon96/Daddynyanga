...
function ProductDetailModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: (data: typeof form) => apiRequest("POST", `/api/products/${product.id}/request`, data),
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 24, width: "min(500px,95vw)", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.2)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800 }}>{product.name}</h3>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#6b7280" }}>✕</button>
        </div>
        {product.imageUrl && <img src={product.imageUrl} alt={product.name} style={{ width: "100%", height: 180, objectFit: "cover", borderRadius: 8, marginBottom: 12 }} />}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, padding: "12px 0", borderTop: "1px solid #e5e7eb" }}>
          <div>
            <p style={{ margin: 0, fontSize: 13, color: "#6b7280" }}>Seller: <strong>{product.sellerName}</strong></p>
            {product.sellerPhone && <p style={{ margin: 0, fontSize: 12, color: "#9ca3af" }}>{product.sellerPhone}</p>}
          </div>
        ...
        <button
          ...
          disabled={mutation.isPending || !form.buyerName || !form.buyerPhone}
...
export default function Marketplace() {
  ...
  const { data: products = [], isLoading } = useQuery<Product[]>({
    ...
    queryFn: () => apiRequest("GET", `/api/products${category !== "All" ? `?category=${category.toLowerCase()}` : ""}`),
  });
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 16 }}>
          {filtered.map(p => (
            <div
              ...
              style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, overflow: "hidden", cursor: "pointer", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", transition: "box-shadow 0.15s" }}
            >
              {p.imageUrl ? (
                <img src={p.imageUrl} alt={p.name} style={{ width: "100%", height: 120, objectFit: "cover", display: "block" }} />
              ) : (
              ...
              <div style={{ padding: "12px 14px" }}>
                ...
                <button
                  style={{ width: "100%", marginTop: 10, padding: "7px", background: "#16a34a", color: "#fff", border: "none", borderRadius: 6, fontWeight: 700, fontSize: 13, cursor: "pointer" }}
                >
                  View & Request
                </button>
              </div>
